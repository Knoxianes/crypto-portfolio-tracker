...
import {  SigninForm, SigninText } from "./components"
  ...
  return (
    <div className="w-full h-screen z-0 absolute top-0 left-0 overflow-hidden">
      ...
      <div className="w-1/2 h-screen absolute top-0 left-0 z-10" >
        <SigninForm className="w-full h-full px-60 py-40" setPopUP={setPopUPForgotPassword} setEmail={setEmail} />
      </div>
      <div className="w-1/2 h-screen absolute top-0 right-0 z-30" >
        <SigninText className="w-full h-full flex justify-center items-center flex-col gap-[10.1rem] text-white px-32" />
      </div>
      <div className="w-1/2 h-screen absolute right-0 top-0 z-20 bg-pink-900"></div>
    </div>
  )
}
